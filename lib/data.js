	...
	fs.open(lib.baseDir+dir+'/'+file+'.json','wx', function(err, fileDescriptor){
		if(!err && fileDescriptor){
			...
			fs.write(fileDescriptor, stringData, function(err){
				if(!err) {
					// And close it - mental note on that one too (the file could not be removed on delete)
...
lib.read = function(dir, file, callback){
	// Open file
	fs.readFile(lib.baseDir+dir+'/'+file+'.json', 'utf-8', function(err, fileData){ // Mental note on that one (remember the charset)
		if(!err && fileData) {
