...
/*
 ...
 Inside user's object orderHistory table is stored Stripe's charge id 
 ...
 hence the order process has not been completed and not removed from system yet
...
helpers.submitStripePayment = function(shoppingCart, callback){
	...
	const token = typeof(shoppingCart.userToken) == 'string' && shoppingCart.userToken.trim().length == 20 ? shoppingCart.userToken : false;
	...
	if(email && shoppingCart) {
		...
		// Unfortunately I coundn't pass the order obj to Stripe's charge metadata property 
		const payload = {
			...
			if(status == 200 || status == 201) {
				...
				res.on('end', function(){
					...
					if(responseBody){
						...
						/* 
							A. get charge.paid, charge.status, charge.id(to store this in user's orderHistory table) using Stripe API endpoint
							and make sure that the Stripe charge returns paid: true, status: SUCCEEDED (DONE)
							
							B. Then move on sending the email receipt. If sent went OK that means that Mailgun has Queued the email for delivery,
							so track email delivery using Mailgun's Event API & emailId from sendMailgunEmailReceipt response obj (DONE)

							C. If email delivery is successfull keep log for the order(@TODO), update users orderHistory collection(DONE) and delete order from filesystem(DONE). 
							@TODO: For any reason that this file couldn't be deleted, should be renamed to <ORDERID>.leftover

								C.1. Workers should check for any leftover orders

							E. @TODO: Orders should be logged daily and zipped inside a logs/dailyorders folder at the end of each day
						...
						*/
						...
						if (stripePaid && stripeStatus) {
							...
							helpers.sendMailgunEmailReceipt(email, 'Pizza delivery - Payment completed', cartString, function(err, mailgunResp){
								if(!err && mailgunResp) {
									// console.log('Mailgun api call went ok:', mailgunResp);
		...
		const evReq = https.request(eventsRequestDetails, function(evRes){
			...
			if(resStatus == 200 || resStatus == 201) {
				...
				evRes.on('data', function(mailgunEvents){
					// eventsResponseBody = helpers.parseJsonToObject(mailgunEvents);
