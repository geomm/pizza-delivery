	...
	if(email && shoppingCart) {
		...
		const req = https.request(requestDetails, function(res){
			...
			if(status == 200 || status == 201) {
				...
				helpers.sendMailgunEmailReceipt(email, 'Pizza delivery - Payment completed', cartString, function(err, mailgunData){
					if(!err && mailgunData) {
						console.log('Mailgun api call went ok: ', mailgunData);

						// Success , pass Stripe response
						callback(false, mailgunData);
						// @TODO: if stripeData and mailgun return success,
						// Delete order from order folder && add to user order history table && find the retrieve uri on the mailgun API 
...
helpers.sendMailgunEmailReceipt = function(to, subject, content, callback){
	...
	if(to && subject && emailHtmlBody) {
		...
		const req = https.request(requestDetails, function(res){
			...
			if(status == 200 || status == 201) {
				// If response is ok pass res object to the callback
				callback(false, res);
			} else {
