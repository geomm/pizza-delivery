...
// Menuitems - post
// Required data: itemName, price, type
// Optional Data: none
...
// ShoppingCarts - post
// Required data: email, cartItems
// Optional data: none
handlers._shoppingCarts.post = function(data, callback){

	const email = typeof(data.payload.email) == 'string' && data.payload.email.trim().length > 0 && data.payload.email.trim().match('@').length == 1 && data.payload.email.trim().match('.').length >= 1 && data.payload.email.trim().match(new RegExp(/\w+@\w+\.\w+/g)) != null ? data.payload.email.trim() : false;
	const cartItems = typeof(data.payload.cartItems) == 'object' && data.payload.cartItems instanceof Array && data.payload.cartItems.length == 0 ? data.payload.cartItems : false; // && data.payload.cartItems.length > 0 
	...
	if(cartId) {
		...
		_data.read('shoppingcarts', cartId, function(err, cartData){
			if(!err && cartData) {
				...
				// AND user is READing his own cart (AND NOT else's)
				handlers._tokens.verifyToken(token, email,function(tokenIsValid){
					if(tokenIsValid) {
...
// ShoppingCarts - put
// Required fields: cartId
// Optional fields: cartItems, proceedPayment (if proceedPayment == true, payment is getting triggered)
handlers._shoppingCarts.put = function(data, callback){
	...
	if(cartId){
		...
		const proceedPayment = typeof(data.payload.proceedPayment) == 'boolean' && data.payload.proceedPayment == false ? false : true;

		// Make sure there is something to update
		if(cartItems || proceedPayment) {
			// Lookup shoppingCart
			_data.read('shoppingcarts', cartId, function(err, cartData){
				if(!err && cartData){
					...
					// Verify that token is valid 
					// AND user is UPDATE(ing) his own cart (AND NOT else's)
					handlers._tokens.verifyToken(token, email, function(tokenIsValid){
						if(tokenIsValid) {

							// Update cartItems
							// @TODO: need a helper function that compares arrays for diff
							if(cartItems && cartItems.length != cartData.items.length){
								cartData.items = cartItems;
							}
							if(proceedPayment) {
								cartData.proceedPayment = proceedPayment;
								if(cartData.proceedPayment == true) {
									// @TODO: Stripe API payment helpers.method should be placed here
									// ...
								}
							}
	...
	if(cartId) {
		...
		_data.read('shoppingcarts', cartId, function(err, cartData){
			...
			if(!err && cartData) {
				...
				// Verify that token is valid 
				// AND user is DELETE(ing) his own cart (AND NOT else's)
				handlers._tokens.verifyToken(token, email, function(tokenIsValid){
					if(tokenIsValid) {
