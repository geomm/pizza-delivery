								...
								if(!err){
									if(cartData.proceedPayment == true){
										...
										handlers._orders.post(cartData, function(res){
											if(res){
												console.log('store order to filesystem STATUS: ',res);
											}
										});
									}
									callback(200);
...
/*------ Orders handler ------ */
/* @WIP - NOTE: orders are not intended to be placed directly by the user BUT by the system authenticated as the user
(user submits his/her cart for payment process),
in any case the stripe charge fails, then the order remains in the orders folder else is getting deleted from orders directory
AND getting logged as succesful order. If order not deleted then order has failed and background workers catch it and handles it.

Total costs are calculated and VAT is applied here
*/
		...
		handlers._tokens.verifyToken(token, email, function(tokenIsValid){
			...
			if(tokenIsValid) {
				...
				_data.read('orders', cartId, function(err, orderData){
					if(err){
						// Here should be placed all the calculations						

						// Delete token before orderObject definition
						delete data.token;
						...
						// Update cart menuitems' info for order submission
									...
									if(!err && itemData) {
										...
										if(itemsUpdated == itemsToUpdate) {
											...
											_data.create('orders', cartId, orderObject, function(err){
												if(!err) {
													// Submit payment to Stripe API & if Stripe charge created succesfully then send email
													// completeResponse is Stripe's & Mailgun's responseBobies respectively in one object
													helpers.submitStripePayment(email, orderObject, function(err, completeResponse){
														if(!err && completeResponse){
															callback(200);
															console.log('Stripe api submission went ok', completeResponse);
															// @TODO: check stripeResponse and if mailgun sent successfully delete order
														} else {
															callback(500, {'Error':'Problem submiting payment to Stripe'});
														}
													});

													// @TODO: delete shopping cart (currently cart is stored as order)
