										...
										if(itemsUpdated == itemsToUpdate) {
											...
											_data.create('orders', cartId, orderObject, function(err){
												if(!err) {
													...
													helpers.submitStripePayment(email, orderObject, function(err, stripeData){
														if(!err && stripeData){
															callback(200);
															console.log('Stripe api submission went ok', stripeData.complete);
															// @TODO: check stripeData and if mailgun sent successfully delete order
														} else {
